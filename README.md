...
- **Closed type:** `type Decision = Restore(Snapshot) | Backup | Deny`; a restore is irreversible
- **Sink / capability:** `grant perform irreversible` → `commit { perform(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/backup-restore/demo.sh
```
...
- `backup-restore_safe.eide` — the correct design.
- `backup-restore_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
