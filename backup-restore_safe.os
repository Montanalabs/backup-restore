#! Backup/restore agent — untrusted a request can only ever become one of a fixed set of decisions over a
...
#! @requires perform — the backup/restore agent sink
#! @effect io
#! @irreversible
#! @taint bridge — extract<Decision> turns the tainted input into a trusted decision
grant perform irreversible

type Snapshot = Nightly | Weekly | Manual
type Decision = Restore(Snapshot) | Backup | Deny
...
commit { perform(d) }  # act on the trusted decision only
